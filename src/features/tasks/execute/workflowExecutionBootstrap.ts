...
import { resolveInteractiveChannels } from './fork/humanGate.js';
...
export interface WorkflowExecutionBootstrap {
  ...
  interactivePermissionPrompt: boolean;
...
export async function createWorkflowExecutionBootstrap(
  ...
  const { headerPrefix = 'Running Workflow:', outputMode = 'terminal' } = options;
  const { interactiveUserInput, interactivePermissionPrompt } = resolveInteractiveChannels(workflowConfig, {
  ...
  const runMetaManager = runBootstrap.publishRunMeta({
    ...
    options: {
      ...
      operationJournalRunSlug,
      operationClaimToken,
      // Fork: stamp the queued task onto the run, so a run can be matched to
      // its task without walking `source_run_slug` back through its ancestors.
      ...(options.traceTaskMetadata?.taskDir
        ? { taskDir: options.traceTaskMetadata.taskDir }
        : {}),
