...
import { humanFacingContent, requestTransitionApproval, stepHasApprovalRule } from './fork/transitionApproval.js';
import type {
  ...
  while (deps.state.status === 'running') {
    ...
    try {
      ...
      if (transition.requiresApproval === true) {
        const approval = await requestTransitionApproval(deps.options.onUserInput, step, response, transition);
        if (approval.kind === 'aborted') {
          abort = abortWorkflow(deps, approval.abortKind, approval.reason);
          break;
        }
...
async function runSingleWorkflowIterationCore(deps: WorkflowRunLoopDeps): Promise<SingleWorkflowIterationResult> {
  ...
  if (transition.requiresApproval === true) {
    const approval = await requestTransitionApproval(deps.options.onUserInput, step, response, transition);
    if (approval.kind === 'aborted') {
      const abort = abortWorkflow(deps, approval.abortKind, approval.reason);
      return { response, nextStep: ABORT_STEP, isComplete: true, loopDetected: loopCheck.isLoop, abort };
    }
    if (approval.kind === 'rejected') {
      deps.addUserInput(approval.feedback);
