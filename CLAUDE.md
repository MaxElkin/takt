...
`src/infra/providers/` exposes a unified `Provider` interface. Registered: `claude-sdk`, `claude` (headless CLI), `codex`, `opencode` (shared server pool), `pi` (Pi SDK), `deepseek-harness` (official Python SDK bridge), `cursor`, `copilot`, `kiro`, `antigravity` (`agy` CLI, fork-only — see `docs/fork/antigravity-provider.md`), `mock`. **Provider errors must surface through `AgentResponse.error`** — otherwise SDK failures appear as empty `blocked` output and are nearly impossible to debug. Use `--provider mock` to exercise the engine without a real API.
